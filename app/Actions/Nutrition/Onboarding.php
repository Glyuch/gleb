...
class Onboarding
{
    ...
    private const QUESTIONS = [
        ...
        2 => 'В каком ты часовом поясе? Пришли город или смещение (по умолчанию Москва — можно написать «Москва»).',
        3 => 'Записал 👌🏻 Теперь параметры: вес, рост, возраст?',
        4 => 'Во сколько обычно встаёшь и ложишься? Напиши два времени, например: 07:00 и 23:00',
        5 => 'Спорт и активность — чем занимаешься и как часто?',
        6 => 'Есть ограничения в еде: аллергии, что не ешь?',
        7 => 'Последнее: заметки о здоровье, если хочешь их учесть (анализы, особенности). Можно пропустить.',
    ];
    ...
    private const LABELS = [
        1 => 'Обращение и цель',
        2 => 'Часовой пояс',
        3 => 'Параметры (вес, рост, возраст)',
        4 => 'Режим дня (подъём/отбой)',
        5 => 'Активность',
        6 => 'Ограничения в еде',
        7 => 'Здоровье',
    ];
    ...
    public function answer(NutritionProfile $profile, string $text, ?int $chatId = null): void
    {
        ...
        if ($step === 2) {
            $this->applyTimezone($profile, $text);
        }
    ...
    /**
     * Шаг 2: ставит часовой пояс из ответа (город/смещение). Нераспознанное —
     * оставляем дефолт Europe/Moscow и идём дальше (анкета не застревает). Ставится
     * до шага режима дня, чтобы подъём/отбой сразу считались в поясе профиля.
     */
    private function applyTimezone(NutritionProfile $profile, string $text): void
    {
        $tz = Timezone::parse($text);
        if ($tz !== null) {
            $profile->timezone = $tz;
            $profile->save();
        }
