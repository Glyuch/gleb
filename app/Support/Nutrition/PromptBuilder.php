...
class PromptBuilder
{
    ...
    private static function persona(?NutritionProfile $profile): string
    {
        ...
        return <<<TXT
            ...
            Никогда не пиши резких окриков или команд капсом («СТОП!», «НЕЛЬЗЯ!»). Даже если клиент прислал запрещёнку или сообщение не по теме — реагируй мягко: сначала тепло прими, затем спокойно подскажи, что лучше в следующий раз, и «почему» через физиологию. Ограничения подавай как временные («на этапе снижения веса»), без осуждения и запугивания.
            ...
            Не утверждай, что приём отмечен/зафиксирован, если в контексте он не помечен как СЪЕДЕН — всегда сверяйся со статусами приёмов (СЪЕДЕН / ЖДЁТ / ПРОПУЩЕН). Поздно съеденный приём — это тот приём, что помечен СЪЕДЕН, а не следующий за ним по времени.
            Всегда начинай ответ с обращения к клиенту по имени в звательной форме («{$name}, …»), тепло и естественно; не повторяй имя дважды в одном сообщении.
            TXT;
    }
    ...
    public static function dayContext(NutritionProfile $profile, CarbonImmutable $date): string
    {
        ...
        foreach (MealPlan::TYPES as $type) {
            ...
            if ($meal === null) {
                $lines[] = "— {$label}: НЕТ ДАННЫХ (приём на сегодня не запланирован).";
            ...
            // Явная словесная метка статуса + окно, чтобы модель не путала поздно
            // съеденный приём с ещё не отмеченным (pending) следующим приёмом.
            ...
            $statusLabel = match ($meal->status) {
                'pending' => 'ЖДЁТ',
                'skipped' => 'ПРОПУЩЕН',
                'missed' => 'НЕ ОТМЕЧЕН',
                default => mb_strtoupper($meal->status),
            };

            $note = match ($meal->status) {
                'pending' => 'ещё не отмечен',
                'skipped' => 'пропущен клиентом',
                'missed' => 'окно прошло, не отмечен',
                default => '',
            };

            $line = "— {$label}: {$statusLabel} (окно {$window}";
            $line .= $note !== '' ? ", {$note})" : ')';
