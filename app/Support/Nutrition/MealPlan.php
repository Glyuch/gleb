...
class MealPlan
{
    ...
    public static function windows(CarbonImmutable $date, array $defaultWindows, array $facts, string $sleepTime): array
    {
        ...
        // Ночной отбой (00:00–03:59) относится к СЛЕДУЮЩИМ суткам. Без сдвига граница
        // «sleep − 2ч» уезжает на предыдущий день, ужин оказывается раньше всех приёмов,
        // и guard ниже придвигает его к концу полдника → фиксированные 16:10 при ЛЮБОМ
        // ночном отбое. Порог — старт окна завтрака (производный от подъёма, который в
        // проекте ограничен 04:00–12:00): ночной отбой всегда ≤ него → +сутки; вечерний
        // (20:00–23:59) всегда > него → те же сутки. Абсурдный полуденный отбой (12:00)
        // остаётся в текущих сутках и обезвреживается guard-ом без инверсии окон.
        $dayStart = $at($defaultWindows['breakfast']['start']);
        if ($sleep->lessThanOrEqualTo($dayStart)) {
            $sleep = $sleep->addDay();
        }
