...
<title>ФондыКвест — анализ выступления</title>
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.1/dist/chart.umd.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/chartjs-plugin-annotation@3.0.1/dist/chartjs-plugin-annotation.min.js"></script>
<style>
  :root{--bg:#0b1020;--card:#141a2e;--ink:#e8ecf6;--muted:#9aa7c7;--line:#283150;
    --accent:#22c55e;--accent2:#0ea5e9;--warn:#f59e0b;--bad:#ef4444;}
  *{box-sizing:border-box}
  ...
  .tldr{background:linear-gradient(135deg,#15203a,#171f33);border:1px solid var(--line);
  ...
  .tldr b{color:#fff}
  .takeaway{font-size:13px;color:#c3cde6;background:rgba(34,197,94,.07);border-left:2px solid var(--accent);
  ...
  .pos{color:#5ee08a}.neg{color:#ff8a8a}
...
<div class="wrap">
<header>
  <h1>ФондыКвест — анализ <span class="r">выступления</span></h1>
...
Chart.defaults.color='#9aa7c7';
Chart.defaults.font.family="-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif";
Chart.defaults.borderColor='#283150';
...
document.getElementById('hdrsub').textContent=
  `Данные на ${D.generated_at} · ${D.registered} регистраций · ${D.total_results} игр · ${D.N} уникальных игроков (по последней попытке)`;
...
new Chart(funnel,{type:'bar',data:{labels:D.funnel.map(f=>f[0]),
  ...
  options:opt({indexAxis:'y',plugins:{legend:{display:false},
    ...
    scales:{x:{beginAtZero:true,grid:{color:'#283150'}},y:{grid:{display:false}}}})});
...
new Chart(scoreHist,{type:'bar',data:{labels:D.score_labels,
  ...
  options:opt({plugins:{legend:{display:false},
    ...
    scales:{y:{beginAtZero:true,grid:{color:'#283150'},title:{display:true,text:'игроков'}},
...
new Chart(bench,{type:'bar',data:{labels:['Средний игрок','Вклад (бенчмарк)','Идеал (max)'],
  ...
  options:opt({plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`${fmt(c.raw)} ₽`}}},
    scales:{y:{beginAtZero:true,grid:{color:'#283150'},ticks:{callback:v=>Math.round(v/1000)+'k'}},
...
new Chart(retLines,{type:'line',data:{labels:D.quarters,
  ...
  options:opt({plugins:{legend:{display:true,position:'bottom'},
    ...
    scales:{y:{grid:{color:'#283150'},ticks:{callback:v=>v+'%'}},x:{grid:{display:false}}}})});
...
new Chart(instrTotal,{type:'bar',data:{labels:D.instr.map(k=>LAB[k]),
  ...
  options:opt({plugins:{legend:{display:false},tooltip:{callbacks:{label:c=>`${c.raw} выборов (${pctOf(c.raw,396)}%)`}}},
    scales:{y:{beginAtZero:true,grid:{color:'#283150'}},x:{grid:{display:false}}}})});
...
new Chart(safeFund,{type:'line',data:{labels:D.quarters,datasets:[
  ...
  options:opt({plugins:{legend:{display:true,position:'bottom'},
    ...
    scales:{y:{beginAtZero:true,max:100,grid:{color:'#283150'},ticks:{callback:v=>v+'%'}},x:{grid:{display:false}}}})});
...
new Chart(chaseStock,{data:{labels:D.quarters,datasets:[
  ...
  options:opt({plugins:{legend:{display:true,position:'bottom'},
    tooltip:{callbacks:{label:c=>`${c.dataset.label}: ${c.raw}%`}}},
    scales:{y:{position:'left',grid:{color:'#283150'},ticks:{callback:v=>v+'%'},title:{display:true,text:'доходность'}},
...
new Chart(byQuarter,{type:'bar',data:{labels:D.quarters,
  ...
  options:opt({plugins:{legend:{display:true,position:'bottom'},
    ...
    scales:{x:{stacked:true,grid:{display:false}},
      y:{stacked:true,max:100,grid:{color:'#283150'},ticks:{callback:v=>v+'%'}}}})});
...
function scatter(canvas,xs,ys,color,xlabel){
  new Chart(canvas,{type:'scatter',data:{datasets:[{data:xs.map((x,i)=>({x:x,y:ys[i]})),
    ...
    options:opt({plugins:{legend:{display:false},
      ...
      scales:{x:{grid:{color:'#283150'},title:{display:true,text:xlabel}},
        y:{grid:{color:'#283150'},title:{display:true,text:'ratio, %'}}}})});
}
...
new Chart(replayBars,{type:'bar',data:{labels:D.replays.map(r=>'#'+r.uid),
  ...
  options:opt({plugins:{legend:{display:true,position:'bottom'},
    ...
    scales:{y:{beginAtZero:true,grid:{color:'#283150'},ticks:{callback:v=>Math.round(v/1000)+'k'}},x:{grid:{display:false}}}})});
...
new Chart(helpedGauge,{type:'doughnut',data:{labels:['Да/Скорее да','Остальные'],
  datasets:[{data:[D.helped_pos,D.helped_ans-D.helped_pos],backgroundColor:[GREEN,'#283150'],borderWidth:0}]},
...
new Chart(readyGauge,{type:'doughnut',data:{labels:['Да/Скорее да','Остальные'],
  datasets:[{data:[D.ready_pos,D.ready_ans-D.ready_pos],backgroundColor:[BLUE,'#283150'],borderWidth:0}]},
  options:opt({cutout:'72%',plugins:{legend:{display:true,position:'bottom'},tooltip:{callbacks:{label:c=>`${c.label}: ${c.raw}`}}}}),
